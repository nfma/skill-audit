...
function scanWithTrivy(resolvedPath: string): Finding[] {
  ...
  try {
        ...
        if (target.Vulnerabilities && target.Vulnerabilities.length > 0) {
          for (const vuln of target.Vulnerabilities) {
            ...
            findings.push({
              ...
              file: target.Target,
              message:
                "[Trivy] Dependency vulnerability in " +
                vuln.PackageName +
                ": " +
                vuln.Title,
    ...
    findings.push({
      ...
      file: resolvedPath,
      message:
        "Trivy scan completed with issues: " +
...
function scanWithOSV(resolvedPath: string): Finding[] {
  ...
  try {
    ...
    const output = execFileSync("osv-scanner", ["--json", "-r", resolvedPath], {
      encoding: "utf-8",
    ...
    findings.push({
      ...
      file: resolvedPath,
      message:
        "OSV scan completed with issues: " +
        (e.message || String(e).slice(0, 100)),
      evidence: e.stack || String(e),
    });
  }
  ...
  for (const lockfile of foundLockfiles) {
    try {
      ...
      findings.push({
        ...
        file: lockfile,
        message:
          "OSV lockfile scan failed: " + (e.message || String(e).slice(0, 100)),
        evidence: e.stack || String(e),
      });
    }
  }
  ...
  for (let i = 0; i < packages.length; i += batchSize) {
    ...
    try {
      ...
      findings.push({
        ...
        file: resolvedPath,
        message:
          "OSV API query failed: " + (e.message || String(e).slice(0, 100)),
        evidence: e.stack || String(e),
      });
    }
  }
...
function extractPackagesFromLockfiles(
  resolvedPath: string,
...
function parseYAMLLockfile(
  ...
  for (const line of lines) {
    ...
    if (yarnMatch) {
      packages.push({
        name: yarnMatch[1],
        version: yarnMatch[2].replace(/^[^0-9]*/, ""),
        ecosystem,
      });
      continue;
    }
  ...
  if (filename === "go.mod") {
    ...
    for (const line of lines) {
      ...
      if (singleMatch) {
        packages.push({
          name: singleMatch[1],
          version: singleMatch[2].replace(/^v/, ""),
          ecosystem,
        });
      }
    }
