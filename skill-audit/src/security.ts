...
function validateContextContract(
    ...
    return [
      {
        ...
        recommendation:
          "Add frontmatter context.reads, context.requires, context.writes, and confirmation fields so agents can reason before invoking shell/tool behavior.",
      },
    ];
  }
  ...
  if (!Array.isArray(contract.reads)) {
    findings.push({
      ...
      recommendation:
        "Declare context.reads with narrow fields such as user_goal, target_environment, or changed_files.",
    });
  }
  if (!Array.isArray(contract.requires)) {
    findings.push({
      ...
      recommendation:
        "Declare context.requires for required user intent, approvals, clean worktree, or verification status.",
    });
  }
  if (!Array.isArray(contract.writes)) {
    findings.push({
      ...
      recommendation:
        "Declare context.writes with compact summary fields such as commands_run, files_changed, and verification_result.",
    });
  }
  if (!contract.confirmation) {
    findings.push({
      ...
      recommendation:
        "Declare confirmation: never, on-risk, or always for shell/tool execution.",
    });
  }
...
function skillCanExecute(manifest: SkillManifest, files: string[]): boolean {
  ...
  return (
    /bash|shell|terminal|run_shell_command|execute|script/i.test(
      allowedTools,
    ) ||
    /```\s*(bash|sh|shell)|\b(npm|npx|bun|pnpm|python|bash|sh)\s+(install|run|exec|[\w.-]+)/i.test(
      content,
    ) ||
