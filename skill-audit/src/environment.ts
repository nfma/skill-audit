...
const SHELL_RULES: PatternRule[] = [
  {
    ...
    severity: "critical",
    asi: "ASI05",
    message: "Shell startup fetches remote script into an interpreter",
    pattern: /(curl|wget)[^\n|]*\|\s*(sh|bash|python|perl)/i,
    recommendation: "Remove remote shell execution from startup files.",
  },
  {
    id: "ENV-SHELL-002",
    severity: "critical",
  ...
  },
  {
    ...
    severity: "high",
    asi: "ASI05",
    message: "Shell alias overrides a sensitive command",
    pattern: /alias\s+(npm|npx|node|git|ssh|curl|wget|python|bash|sh)=/i,
    recommendation:
      "Review aliases that shadow common development and network tools.",
  },
  {
    id: "ENV-SHELL-004",
    severity: "medium",
    asi: "ASI04",
    message: "Shell startup exports a likely secret",
    pattern: /export\s+\w*(TOKEN|SECRET|PASSWORD|API_KEY)\w*=/i,
    recommendation:
      "Avoid storing credentials directly in shell startup files.",
  },
];
...
const AGENT_CONFIG_RULES: PatternRule[] = [
  {
    ...
    severity: "high",
    asi: "ASI05",
    message: "Agent hook executes a shell command",
    pattern: /"(PreToolUse|PostToolUse|hooks?)"[\s\S]{0,800}"command"\s*:/i,
    recommendation:
      "Verify hook commands are expected, pinned, and documented.",
  },
  {
    id: "ENV-HOOK-002",
    severity: "critical",
    asi: "ASI05",
    message: "Agent config contains remote script execution",
    pattern: /(curl|wget)[^\n|]*\|\s*(sh|bash|python|perl)/i,
  },
  {
    id: "ENV-MCP-001",
    severity: "medium",
    asi: "ASI02",
    message: "MCP/tool server uses unpinned npx latest-style execution",
    pattern: /"command"\s*:\s*"npx"[\s\S]{0,400}(@latest|"latest"|\s-y\s)/i,
    recommendation:
      "Pin MCP/tool package versions instead of using latest or implicit installs.",
  },
  {
    id: "ENV-SECRET-001",
    severity: "high",
    asi: "ASI04",
    message: "Agent config appears to contain a secret value",
    pattern: /"\w*(TOKEN|SECRET|PASSWORD|API_KEY)\w*"\s*:\s*"[^"$][^"]{8,}"/i,
    recommendation:
      "Move secrets to a secret manager or environment source outside agent config.",
  },
];
...
const INSTRUCTION_RULES: PatternRule[] = [
  {
    ...
    severity: "critical",
    asi: "ASI01",
    message: "Agent instruction file attempts to override prior instructions",
  ...
  },
  {
    ...
    severity: "high",
    asi: "ASI01",
    message: "Agent instruction file contains context-forgetting directive",
  ...
  },
  {
    ...
    severity: "high",
    asi: "ASI04",
    message: "Agent instruction file asks for secrets or credentials",
    pattern:
      /(reveal|print|exfiltrate|send).{0,80}(secret|token|password|api key|credential)/i,
  },
  {
    id: "ENV-CTX-004",
    severity: "medium",
...
const PACKAGE_RULES: PatternRule[] = [
  {
    ...
    severity: "critical",
    asi: "ASI05",
    message: "Package lifecycle script fetches remote code into shell",
    pattern:
      /"(preinstall|install|postinstall|prepare)"\s*:\s*"[^"]*(curl|wget)[^"|]*\|\s*(sh|bash|python|perl)/i,
  },
  {
    id: "ENV-PKG-002",
    severity: "high",
    asi: "ASI05",
    message: "Package lifecycle script contains destructive filesystem command",
    pattern:
      /"(preinstall|install|postinstall|prepare)"\s*:\s*"[^"]*rm\s+-rf\s+(\/|~|\$HOME)/i,
  },
  {
    id: "ENV-PKG-003",
    severity: "medium",
    asi: "ASI04",
    message: "Package lifecycle script accesses likely secrets",
    pattern:
      /"(preinstall|install|postinstall|prepare)"\s*:\s*"[^"]*(TOKEN|SECRET|PASSWORD|API_KEY|\.env)/i,
  },
];

const SENSITIVE_COMMAND_PATTERNS = [
  {
    reason: "skill installation command",
    pattern: /\bnpx\s+skills\s+add\b|\bskills\s+add\b/i,
  },
  {
    reason: "package install command",
    pattern: /\b(npm|pnpm|bun|yarn)\s+(install|add|i)\b/i,
  },
  {
...
export function runEnvironmentDoctor(
  ...
  summary.agentConfigFiles = scanFiles(
    agentConfigFiles,
    AGENT_CONFIG_RULES,
    findings,
  );
  summary.instructionFiles = scanFiles(
    instructionFiles,
    INSTRUCTION_RULES,
    findings,
  );
...
export function writeEnvironmentBaseline(
  options: EnvironmentDoctorOptions = {},
): EnvironmentBaseline {
...
export function diffEnvironmentBaseline(
  options: EnvironmentDoctorOptions = {},
  ...
  const baselineFiles = new Map(
    baseline.result.files.map((file) => [file.path, file.sha256]),
  );
  const currentFiles = new Map(
    current.files.map((file) => [file.path, file.sha256]),
  );
  const addedFiles = current.files
    .filter((file) => !baselineFiles.has(file.path))
    .map((file) => file.path);
  const removedFiles = baseline.result.files
    .filter((file) => !currentFiles.has(file.path))
    .map((file) => file.path);
  const changedFiles = current.files
    .filter(
      (file) =>
        baselineFiles.has(file.path) &&
        baselineFiles.get(file.path) !== file.sha256,
    )
    .map((file) => file.path);
  ...
  const addedFindings = current.findings.filter(
    (finding) => !baselineFindings.has(findingKey(finding)),
  );
  const resolvedFindings = baseline.result.findings.filter(
    (finding) => !currentFindings.has(findingKey(finding)),
  );
...
export function assessShellCommand(
  command: string,
  options: EnvironmentDoctorOptions = {},
): EnvironmentCommandAssessment {
  const reasons = SENSITIVE_COMMAND_PATTERNS.filter((rule) =>
...
export function reportCommandAssessment(
  assessment: EnvironmentCommandAssessment,
  options: {
    json?: boolean;
    verbose?: boolean;
    block?: boolean;
  ...
  console.log(
    `   Environment drift: ${assessment.environment.drift ? "⚠️ yes" : "✅ no"}`,
  );
  console.log(
    `   Environment risk: ${assessment.environment.current.riskLevel} (${assessment.environment.current.riskScore.toFixed(1)}/10)`,
  );
  ...
  for (const path of paths) {
    ...
    for (const rule of rules) {
      ...
      findings.push({
        ...
        evidence: rule.id.includes("SECRET")
          ? redactSecretEvidence(match[0])
          : trimEvidence(match[0]),
  ...
  for (const entry of pathValue.split(delimiter).filter(Boolean)) {
    if (isWorldWritable(entry)) {
      findings.push({
        ...
        recommendation:
          "Remove world-writable directories from PATH or move them after trusted system paths.",
      });
    }
  }
    ...
    for (const resolvedPath of whichAll(bin, pathValue)) {
      if (isWorldWritable(resolvedPath)) {
        findings.push({
          ...
          recommendation:
            "Use trusted, non-writable tool binaries for agent shell execution.",
        });
      }
      if (resolve(resolvedPath).startsWith(resolve(cwd))) {
        findings.push({
          ...
          recommendation:
            "Check for PATH hijacking before running agent shell commands.",
        });
      }
    }
  }
...
export function reportEnvironmentDoctor(
  ...
  if (!options.verbose && result.findings.length > shown.length) {
    console.log(
      `   ... and ${result.findings.length - shown.length} more findings. Re-run with --verbose for details.`,
    );
  }
}

export function reportEnvironmentBaseline(
  baseline: EnvironmentBaseline,
  baselinePath: string,
  ...
  console.log(
    `   Risk: ${baseline.result.riskLevel} (${baseline.result.riskScore.toFixed(1)}/10)`,
  );
...
export function reportEnvironmentDiff(
  ...
  console.log(
    `   Current risk: ${diff.current.riskLevel} (${diff.current.riskScore.toFixed(1)}/10)`,
  );
  console.log(
    `   Added files: ${diff.addedFiles.length} | Removed files: ${diff.removedFiles.length} | Changed files: ${diff.changedFiles.length}`,
  );
  console.log(
    `   Added findings: ${diff.addedFindings.length} | Resolved findings: ${diff.resolvedFindings.length}\n`,
  );

  if (!options.verbose) {
    if (diff.drift)
      console.log("   Re-run with --verbose for changed files and findings.\n");
    return;
  }

  for (const file of diff.addedFiles)
    console.log(`   + file ${displayPath(file)}`);
